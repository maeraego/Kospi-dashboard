...
왜 필요한가
-----------
평소 배포는 GitHub Pages(공개)로 하지만, 특정인에게 파일로 건네고 싶을 때가 있다.
그럴 때 카톡·메일에 바로 얹을 수 있는 한 장을 만들어 둔다. 자동 실행에는 들어있지 않으므로
필요할 때 직접 실행한다:  python build_dashboard.py  →  python make_share.py
