...
# ── 공포탐욕지수 (Fear & Greed) — 참고지표. 가중 0 ──
#   CNN 공포탐욕지수의 구성요소를 이 프로젝트 데이터로 재현한 것(fear_greed.py).
#   CNN 원본은 2011년~이고 과거값을 공개하지 않아 20년 검증이 불가능하다.
#   [왜 가중 0인가] 구간별 성과는 뚜렷하다 — 극단적 공포 진입 후 1년 +24.3%(승률 76%),
#     극단적 탐욕 +0.5%(50%). 그런데 선형 IC는 한국 0.086 · 미국 0.013으로 편입기준
#     0.10에 미달한다. 극단만 쓰는 변형(25/75, 20/80), 제곱, 순수심리 4요소까지
#     전부 0.042~0.086으로 탈락했다.
#   [이유] 공포탐욕의 예측력 있는 재료가 이미 개별 신호로 대시보드에 들어와 있다 —
#     변동성(VKOSPI) · 레버리지(신용융자/예탁금) · 신용스프레드. 공포탐욕은 여기에
#     모멘텀·52주위치를 섞는데, 이 둘은 코스피 그 자체라 동행할 뿐 앞서지 않는다
#     (실측 CCF ccf0 0.708~0.775, peak_lag의 ccf는 0.016~0.036으로 사실상 0).
#     즉 예측 재료는 중복이고, 추가된 재료는 동행지표다.
#   → 시장 심리를 눈으로 보는 용도로 차트에만 남긴다.
try:
    _fgp = 'fear_greed_daily.parquet'
    _fg = load(_fgp)
    for _c, _lab in (('KR_공포탐욕', '공포탐욕(한국)'), ('US_공포탐욕', '공포탐욕(미국)')):
        _s = _fg[_c].dropna()
        df[_lab] = _s.resample('ME').last().reindex(df.index)
        _l = _s.index[-1].to_period('M').to_timestamp('M')
        if _l in df.index:                     # 최신 월은 월말 전이라도 최근값으로 덮어씀
            df.loc[_l, _lab] = float(_s.iloc[-1])
        COL_SRC[_lab] = '자체 산출'
    HAS_FG = int(df['공포탐욕(한국)'].notna().sum()) >= 60
    if HAS_FG:
        FILE_INFO.append(('자체 산출', 'CNN 방식 재현 (fear_greed.py)', _fgp,
                          pd.Timestamp(os.path.getmtime(os.path.join(HERE, _fgp)),
                                       unit='s', tz='UTC').tz_convert('Asia/Seoul')
                            .strftime('%Y-%m-%d %H:%M')))
except Exception:
    HAS_FG = False
...
def signals_for(idx):
    ...
    # ── 신규 후보 (check_newsig.py 로 IC/CCF/중복도 판정) ──
    #   채산성 = 소비자물가 상승률 − 생산자물가 상승률.
    #     기업이 파는 값보다 사는 값이 덜 오르면 마진이 개선된다는 발상.
    #     실측 IC 코스피 +0.05 / 코스닥 −0.10 으로 약해 REF_ONLY 로 둔다.
    #     CCF 상 지수보다 늦게 움직여(최대상관 lag −6~−8개월) 선행지표가 아니다.
    _margin = ((np.log(df['CPI']).diff(12) - np.log(df['PPI']).diff(12)) * 100
               if ('CPI' in df and 'PPI' in df) else None)
    #   원/엔: IC 코스피 +0.243 로 후보 중 가장 강하고, 기존 신호와 최대 상관이
    #     0.44 에 그쳐 겹치지 않는다. 엔 캐리 자금의 방향을 재는 축.
    _jpy = df['원엔100'] if '원엔100' in df else None
    #   원/위안: IC +0.21 이지만 원/달러와 0.687 겹친다. 가중은 주되 중복벌점에 맡긴다.
    _cny = df['원위안'] if '원위안' in df else None
    #   일본10년: IC 는 기준을 넘지만 국고채10년과 0.806, 일드갭과 −0.808 로
    #     사실상 같은 말을 두 번 하게 된다. 평균상관 기반 벌점은 이런 '한 쌍만
    #     강한 겹침'을 희석해 못 잡으므로(CLAUDE.md 4항) REF_ONLY 로 고정한다.
    _jp10 = df['JP10Y'] if 'JP10Y' in df else None
    # ── 통화량(M1/M2) 파생 신호 ──
    ...
    sig = [
        ...
        ('환율(원/엔100)', _jpy,                       +1, FMT_WON, ('엔화강세', '엔화약세')),
        ('환율(원/위안)',  _cny,                       +1, FMT_WON, ('위안강세', '위안약세')),
        ...
        ('기준금리(韓)',   df['기준금리'],             -1, FMT_RATE, ('고금리', '저금리')),
    ...
    if idx == 'KOSPI' and HAS_FG:
        # 참고지표 — REF_ONLY라 가중 0. 차트에만 나온다.
        sig.append(('공포탐욕(한국)', df['공포탐욕(한국)'], -1, FMT_1, ('탐욕', '공포')))
        sig.append(('공포탐욕(미국)', df['공포탐욕(미국)'], -1, FMT_1, ('탐욕', '공포')))
...
#
#   [예상PER 계열은 제외한다 - 재검토 결과]
#   update_fwd_per.py 의 ni_mode 는 asof = mc.index[-1] 로, 입력한 컨센서스를
#   '입력한 그 달'에 찍는다. 즉 발표시차가 존재하지 않는 값이다.
#   시차를 주면 오늘 넣은 컨센서스가 2개월 뒤에야 점수에 반영돼,
#   사용자가 최신 추정치를 넣어도 화면이 안 움직이는 문제가 생긴다
#   (2026-09 FY2 964조 입력 시 실제로 확인).
#   표본외 IC 는 시차를 줄 때 +0.031 좋았지만 그건 컨센서스가 천천히
#   움직여 생기는 선후행 효과지 룩어헤드 보정이 아니다. 기전이 없는 보정을
#   성적만 보고 넣지 않는다. 경기선행지수(공식 2개월 시차)만 남긴다.
PUB_LAG = {
...
def analyze(idx):
    ...
    REF_ONLY = REF_ONLY | {'공포탐욕(한국)', '공포탐욕(미국)'}
    # ── 신규 4종은 전부 REF_ONLY (차트만, 가중 0) ──
    #   _oos_new.py 로 2015년 분할 표본외 IC 를 재본 결과:
    #     코스피  기존 +0.7425 → 채산성 −0.0040 · 원/위안 −0.0016 · 전부포함 −0.0048
    #     코스닥  기존 +0.6315 → 국채10년(日) +0.0049 · 원/위안 +0.0047 · 전부포함 +0.0092
    #   코스피는 넷 다 무익하거나 해롭고 코스닥만 +0.005 수준으로 개선되는데
    #   이는 노이즈 범위다. 지수별로 다른 신호를 쓰는 복잡도를 지불할 만한
    #   크기가 아니라고 보고 전부 차트 전용으로 둔다.
    #   개별 사유:
    #     채산성        CCF 상 지수를 6~8개월 후행한다(선행지표가 아님).
    #     국채10년(日)  국고채10년과 0.806, 일드갭과 −0.808 로 사실상 중복.
    #                   평균상관 벌점은 '한 쌍만 강한 겹침'을 희석해 못 잡는다.
    #     원/엔         원계열 IC 0.243 은 추세 착시였고 expanding z 후 0.084 로 탈락.
    #     원/위안       원/달러와 0.687 중복. 코스피 표본외를 오히려 낮춘다.
    REF_ONLY = REF_ONLY | {'채산성(CPI-PPI)', '국채10년(日)',
                           '환율(원/엔100)', '환율(원/위안)'}
...
USED_COLS.discard('공포탐욕(한국)'); USED_COLS.discard('공포탐욕(미국)')
...
comp = {
    ...
    '일드커브(韓)': df['국고채10년'] - df['국고채3년'],
    '일드갭(韓)': (100/df['예상PER'].where(df['예상PER'] > 0) - df['국고채10년']) if HAS_FPE
              else (100/df['KOSPI_PER'].where(df['KOSPI_PER'] > 0) - df['국고채10년']),
    '국채10년(韓)': df['국고채10년'], '국채3년(韓)': df['국고채3년'],
    '기준금리(韓)': df['기준금리'], '환율(원/달러)': df['원달러'], '실현변동성(20일)': df['KOSPI_변동성'], 'VKOSPI': df.get('VKOSPI'),
    '환율(원/엔100)': df.get('원엔100'), '환율(원/위안)': df.get('원위안'),
    '국채10년(日)': df.get('JP10Y'),
    '채산성(CPI-PPI)': ((np.log(df['CPI']).diff(12) - np.log(df['PPI']).diff(12)) * 100
                       if ('CPI' in df and 'PPI' in df) else None),
    ...
    '공포탐욕(한국)': df.get('공포탐욕(한국)'), '공포탐욕(미국)': df.get('공포탐욕(미국)'),
...
_SIG2COL2 = {'PBR': 'PBR', 'PER': 'PER', 'ROE': 'ROE', '실현변동성(20일)': '실현변동성(20일)', 'VKOSPI': 'VKOSPI',
             ...
             '선행 PBR': '선행 PBR',
             '환율(원/달러)': '환율(원/달러)', '경기선행지수': '경기선행지수(韓)', '신용스프레드(韓)': '신용스프레드(韓)',
             '일드커브(韓)': '일드커브(韓)', '국채10년(韓)': '국채10년(韓)',
             'WTI 유가': 'WTI 유가', '국채10년 YoY(美)': '국채10년 YoY(美)',
             'VIX 급등(YoY)(美)': 'VIX 급등(YoY)(美)', '기준금리 YoY(韓)': '기준금리 YoY(韓)',
             '기준금리(韓)': '기준금리(韓)',
             '수출 YoY': '순수출', '예상PER 괴리': '예상PER',
             '시가총액/M2': '시가총액/M2', 'M2 증가율(YoY)': 'M2 증가율', 'M2/M1 비율': 'M2/M1 비율',
             '신용융자/예탁금': '신용융자/예탁금',
             '공포탐욕(한국)': '공포탐욕(한국)', '공포탐욕(미국)': '공포탐욕(미국)',
             '일드갭 (예상PER)': '일드갭(韓)', '일드갭 (후행PER)': '일드갭(韓)',
           '환율(원/엔100)': '환율(원/엔100)', '환율(원/위안)': '환율(원/위안)',
           '채산성(CPI-PPI)': '채산성(CPI-PPI)', '국채10년(日)': '국채10년(日)'}
DATA['ic'] = {}
for _n in AK['ic']:
