...
S["원엔100"]      = collect("원/엔(100엔)", "731Y001", "D", [r"원/일본엔"])
# 원/위안은 계열이 두 번 바뀐다. 구계열(0000027, 1994~2016.02)과
# 신계열(0000053 매매기준율, 2016.01~)을 이어붙여야 장기 시계열이 된다.
_cny_new = collect("원/위안(신)", "731Y001", "D", [r"원/위안\(매매기준율\)"])
_cny_old = collect("원/위안(구)", "731Y001", "D", [r"원/중국위안"])
if _cny_new is not None and _cny_old is not None:
    S["원위안"] = _cny_new.combine_first(_cny_old).rename("원위안")
    print(f"  · 원/위안(접합)   <- 신계열 우선 + 구계열 보충  n={S['원위안'].dropna().shape[0]}")
elif _cny_new is not None:
    S["원위안"] = _cny_new.rename("원위안")

# 물가 (채산성 = 소비자물가상승률 − 생산자물가상승률)
#   기업이 파는 값(PPI)보다 사는 값(CPI)이 덜 오르면 마진이 개선된다는 발상.
#   원자재·중간재 가격 압력 대비 최종재 전가력을 재는 대용치.
S["CPI"] = collect("소비자물가", "901Y009", "M", [r"^총지수$"])
S["PPI"] = collect("생산자물가", "404Y014", "M", [r"^총지수$"])
...
#     161Y006 = M2(평잔, 원계열).  BBHS00=M2 총액,
#     BBHS01=현금통화, BBHS02=요구불예금, BBHS03=수시입출식저축성예금
#   M1(협의통화) = 현금통화 + 요구불예금 + 수시입출식저축성예금
#   M2/M1 비율 = 유동성 회전속도. M2 증가율 = 대표 경기 선행지표.
_M2TBL = "161Y006"
_cash = collect("현금통화",   _M2TBL, "M", [r"현금통화"])
_dd   = collect("요구불예금",  _M2TBL, "M", [r"요구불예금"])
_mmda = collect("수시입출식",  _M2TBL, "M", [r"수시입출식"])
...
MONTHLY_KEYS = {"선행지수", "수출금액", "수출금액지수", "무역수지", "M1", "M2", "CPI", "PPI"}  # 월별 네이티브
